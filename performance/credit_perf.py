"""Credit performance dashboard generation with price‑mode awareness and source footnotes.

This module produces charts summarising recent performance for a selection
of credit indices (investment‑grade and high‑yield).  Users can choose
to base calculations on either the most recent intraday price ("Last Price")
or the previous day's closing price ("Last Close").  The resulting
...
* ``create_historical_performance_table`` – Build the heatmap of
  ...
  effective date.
* ``insert_credit_performance_bar_slide`` – Insert the weekly bar chart
  and source footnote into its slide.
* ``insert_credit_performance_histo_slide`` – Insert the historical
...
Usage example::
    ...
    prs = insert_credit_performance_bar_slide(prs, bar_bytes, used_date, price_mode)
    prs = insert_credit_performance_histo_slide(prs, histo_bytes, used_date, price_mode)
"""
...
def _load_price_data(excel_path: Union[str, pathlib.Path], tickers: List[str]) -> pd.DataFrame:
    """Load a DataFrame of dates and prices for the specified credit tickers.
    ...
    tickers : list of str
        Column names in the sheet corresponding to the desired credit indices.
...
def _build_returns_table(
    ...
    """Calculate performance metrics for each credit ticker.
...
###############################################################################
# Colour helper (green for positive, red for negative)
###############################################################################
...
def create_weekly_performance_chart(
    ...
    *,
    ...
    ``"Last Close"``), computes 1‑week returns for the configured credit
    indices, sorts them in descending order and constructs a horizontal bar
    chart.  A positive return is depicted in green; a negative return in
    red.  The effective date used for the calculations is returned along
    with the PNG data.
    ...
    """
    default_mapping = {
        "LUACTRUU Index": "USD - IG",
        "LF98TRUU Index": "USD - HY",
        "LECPTREU Index": "EUR - IG",
        "IBOXXMJA Index": "EUR - HY",
        "IBXXAEJT Index": "Asia (ex JP) - IG",
        "IBXXAHBI Index": "Asia - HY",
        "JPEIESGE Index": "EM - IG",
        "JPEIEMHY Index": "EM - HY",
    }
    ...
    # Determine a dynamic offset for label placement relative to bar length.  Use a
    # small fraction of the total range so labels stay close to the end of the bar.
...
def create_historical_performance_table(
    ...
    *,
    ...
    """Generate a heatmap table of returns with price‑mode adjustment.
    ...
    positive values map to green and negative values to red.  The
...
def _insert_dashboard_to_placeholder(
    ...
    *,
    ...
    placeholder_names : list of str
        ...
        recognises text placeholders like ``[credit_perf_1week]``.
...
def insert_credit_performance_bar_slide(
    prs: Presentation,
    image_bytes: bytes,
    ...
    *,
    ...
    """Insert the weekly credit bar chart and source footnote into its slide.

    The slide is identified by a shape named ``credit_perf_1week`` or
    ``credit_perf_1w`` (or containing ``[credit_perf_1week]``).  The
    chart is inserted at the specified coordinates and a source footnote is
    written into ``credit_1w_source`` or a shape containing ``[credit_1w_source]``.
    ...
    return _insert_dashboard_to_placeholder(
        prs,
        image_bytes,
        placeholder_names=["credit_perf_1w", "credit_perf_1week"],
        left_cm=left_cm,
        top_cm=top_cm,
        width_cm=width_cm,
        height_cm=height_cm,
        ...
        source_placeholder_names=["credit_1w_source"],
    )


def insert_credit_performance_histo_slide(
    prs: Presentation,
    image_bytes: bytes,
    used_date: Optional[pd.Timestamp] = None,
    price_mode: str = "Last Price",
    *,
    ...
    """Insert the credit historical performance heatmap and source footnote into its slide.

    The slide is identified by a shape named ``credit_perf_histo`` (or
    containing ``[credit_perf_histo]``).  A source footnote is written
    into ``credit_1w_source2`` or a shape containing ``[credit_1w_source2]``.
    ...
    return _insert_dashboard_to_placeholder(
        prs,
        image_bytes,
        placeholder_names=["credit_perf_histo"],
        left_cm=left_cm,
        top_cm=top_cm,
        width_cm=width_cm,
        height_cm=height_cm,
        used_date=used_date,
        price_mode=price_mode,
        source_placeholder_names=["credit_1w_source2"],
    )