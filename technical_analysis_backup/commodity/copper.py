...
def _get_copper_technical_score(excel_obj_or_path) -> Optional[float]:
    """
    Retrieve the technical score for Copper from 'data_technical_score' (col A, B).
    ...
    for _, row in df.iterrows():
        if str(row[df.columns[0]]).strip().upper() == "LP1 COMDTY":
            try:
...
def generate_range_callout_chart_image(
    ...
    *,
    ...
    """
    Create a PNG image of the Copper price chart with a textual call‑out on the
    ...
    df_full : pandas.DataFrame
        Full Copper price history with 'Date' and 'Price' columns.
    anchor_date : pandas.Timestamp or None, optional
    ...
    # value is provided (e.g. the LPR1 Index level), use it to estimate the
    ...
    ax_chart.plot(df["Date"], df["Price"], color="#153D64", linewidth=2.5,
                  label=f"Copper Price (last: {last_price:,.2f})")
...
def _get_copper_momentum_score(excel_obj_or_path) -> Optional[float]:
    """Return Copper momentum score, mapping letter grades to numeric if needed."""
    try:
        df = pd.read_excel(excel_obj_or_path, sheet_name="data_trend_rating")
    except Exception:
        return None
    # find Copper row
    mask = df.iloc[:, 0].astype(str).str.strip().str.upper() == "LP1 COMDTY"
    if not mask.any():
        return None
    ...
    try:
        ...
        cu_param = params[params["Tickers"].astype(str).str.upper() == "LP1 COMDTY"]
        if not cu_param.empty and "Unnamed: 8" in cu_param.columns:
            custom_series = cu_param["Unnamed: 8"].dropna()
...
def _compute_range_bounds(
    ...
    """
    Compute fallback high and low range bounds for the Copper using
    realised volatility.

    This helper is used when an implied volatility index (e.g. LPR1 Index) is
...
def generate_range_gauge_chart_image(
    ...
    *,
    ...
    """
    Create a PNG image of the Copper price chart with a vertical range gauge
    ...
    ----------
    df_full : pandas.DataFrame
        Full Copper price history as returned by ``_load_price_data``.
    anchor_date : pandas.Timestamp or None, optional
    ...
    df = df_full[df_full["Date"].between(start, today)].reset_index(drop=True)
    # Compute moving averages on the full dataset and then slice to the
    # plotting window.  This preserves the correct lookback for long-term
    # averages (e.g. 200-day) when only a short window of data is displayed.
    df_ma_full = _add_mas(df_full)
    df_ma = df_ma_full[df_ma_full["Date"].between(start, today)].reset_index(drop=True)
    ...
    ax.plot(
        df["Date"], df["Price"], color="#153D64", linewidth=2.5, label=f"Copper Price (last: {last_price_str})"
    )
