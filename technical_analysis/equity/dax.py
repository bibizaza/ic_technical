...
def insert_dax_technical_chart_with_callout(
    ...
    img_bytes = generate_range_callout_chart_image(
        ...
        show_legend=False,
        cache_key="dax_main_callout",  # Phase 2: Use cached chart
    )
    ...
    # ``last_price_dax``.  Font attributes are preserved.
    last_price = None
    ...
    placeholder_name = "last_price_dax"
    placeholder_patterns = ["[last_price_dax]", "last_price_dax"]
    replaced = False
...
def insert_dax_average_gauge(
    ...
    try:
        gauge_bytes = generate_average_gauge_image(
            ...
            width_cm=15.15,
            height_cm=3.13,
            cache_key="dax_avg_gauge",  # Phase 2: Use cached gauge
        )
    except Exception:
        return prs
    # Identify DAX slide
