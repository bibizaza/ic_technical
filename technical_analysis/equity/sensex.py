...
def _find_sensex_slide(prs: Presentation) -> Optional[int]:
    """Find the SENSEX slide by placeholder."""
    return find_slide_by_placeholder(prs, "sensex")
...
def insert_sensex_technical_score_number(prs: Presentation, excel_file) -> Presentation:
    ...
    return insert_score_number(prs, score, "sensex", "tech_score")
...
def insert_sensex_technical_chart_with_callout(
    ...
    img_bytes = generate_range_callout_chart_image(
        ...
        show_legend=False,
        cache_key="sensex_main_callout",  # Phase 2: Use cached chart
    )
...
def insert_sensex_momentum_score_number(prs: Presentation, excel_file) -> Presentation:
    ...
    return insert_score_number(prs, score, "sensex", "momentum_score")
...
def insert_sensex_subtitle(prs: Presentation, subtitle: str) -> Presentation:
    """Insert subtitle into the SENSEX slide."""
    return insert_subtitle(prs, subtitle, "sensex")
...
def insert_sensex_average_gauge(
    ...
    try:
        gauge_bytes = generate_average_gauge_image(
            ...
            width_cm=15.15,
            height_cm=3.13,
            cache_key="sensex_avg_gauge",  # Phase 2: Use cached gauge
        )
    except Exception:
        return prs
