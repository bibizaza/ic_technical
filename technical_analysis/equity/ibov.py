"""
Refactored Bovespa technical analysis module using base instrument class.
...
- Reduced code from 2233+ lines to ~300 lines
"""
...
# Import MARS scorer
try:
    from mars_engine.mars_lite_scorer import generate_ibov_score_history
except Exception:
    generate_ibov_score_history = None
...
_config = InstrumentConfig(
    ...
    mars_scorer_func=generate_ibov_score_history,
)
...
def make_ibov_figure(
    ...
    """Build an interactive Bovespa chart for Streamlit."""
...
def insert_ibov_technical_chart(
    ...
    """Insert a technical chart into the Bovespa slide."""
...
def insert_ibov_technical_score_number(prs: Presentation, excel_file) -> Presentation:
    """Insert the Bovespa technical score into the slide."""
    return _instrument.insert_technical_score_number(prs, excel_file)


def insert_ibov_momentum_score_number(
    prs: Presentation,
    excel_file,
    price_mode: str = "Last Price",
) -> Presentation:
    """Insert the Bovespa momentum score into the slide."""
...
def insert_ibov_subtitle(prs: Presentation, subtitle: str) -> Presentation:
    """Insert a subtitle into the Bovespa slide."""
    return _instrument.insert_subtitle(prs, subtitle)


def _get_ibov_technical_score(excel_obj_or_path) -> Optional[float]:
    """Retrieve the technical score for Bovespa."""
    return _instrument._get_technical_score(excel_obj_or_path)


def _get_ibov_momentum_score(
    excel_obj_or_path,
    price_mode: str = "Last Price",
) -> Optional[float]:
    """Retrieve the MARS momentum score for Bovespa."""
    return _instrument._get_momentum_score(excel_obj_or_path, price_mode)


# Stub functions for compatibility
def insert_ibov_technical_chart_with_callout(prs: Presentation, *args, **kwargs) -> Presentation:
    """Insert chart with callout (stub)."""
    return insert_ibov_technical_chart(prs, *args, **kwargs)


def insert_ibov_technical_chart_with_range(prs: Presentation, *args, **kwargs) -> Presentation:
    """Insert chart with range gauge (stub)."""
    return insert_ibov_technical_chart(prs, *args, **kwargs)


def insert_ibov_average_gauge(prs: Presentation, *args, **kwargs) -> Presentation:
    """Insert average gauge (stub)."""
    return prs


def insert_ibov_technical_assessment(prs: Presentation, *args, **kwargs) -> Presentation:
    """Insert technical assessment (stub)."""
    return prs


def insert_ibov_source(prs: Presentation, *args, **kwargs) -> Presentation:
