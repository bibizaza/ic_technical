"""
Utility functions for IBOV technical analysis and high‑resolution export.

This module provides tools to build interactive and static charts for the
IBOV index, calculate and insert technical and momentum scores into
...
* ``make_ibov_figure`` – interactive Plotly chart for Streamlit.
* ``insert_ibov_technical_chart`` – insert a static IBOV chart into a PPTX.
* ``insert_ibov_technical_score_number`` – insert the technical score (integer).
* ``insert_ibov_momentum_score_number`` – insert the momentum score (integer).
* ``insert_ibov_subtitle`` – insert a user‑defined subtitle into the IBOV slide.
* ``generate_average_gauge_image`` – create a horizontal gauge image.
* ``insert_ibov_average_gauge`` – insert the gauge into a PPT slide.
* ``insert_ibov_technical_assessment`` – insert a descriptive “view” text.
* ``generate_range_gauge_chart_image`` – create a combined price chart with
  ...
  ``insert_ibov_technical_chart_with_range``.
* ``insert_ibov_technical_chart_with_range`` – insert the IBOV technical
  analysis chart with the higher/lower range gauge into the PPT.

The range gauge illustrates the recent trading range for the IBOV.
...
code looks up the forward‑looking volatility index (IBOV) and computes a
1‑week expected move as ``(current_price × (IBOV / 100)) / sqrt(52)``.
...
def _get_vol_index_value(
    excel_obj_or_path,
    price_mode: str = "Last Price",
    vol_ticker: str = "IBOV Index",
) -> Optional[float]:
    """
    Retrieve the most recent value of a volatility index (e.g. IBOV) from
    ...
    vol_ticker : str, default "IBOV Index"
...
def make_ibov_figure(
    excel_path: str | pathlib.Path,
    anchor_date: Optional[pd.Timestamp] = None,
    price_mode: str = "Last Price",
) -> go.Figure:
    """
    Build an interactive IBOV chart for Streamlit.

    Parameters
    ----------
    excel_path : str or pathlib.Path
        Path to the Excel file containing IBOV price data.
    ...
    df_raw = _load_price_data(excel_path, "IBOV Index", price_mode=price_mode)
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=df["Price"],
            mode="lines",
            name=f"IBOV Price (last: {last_price_str})",
...
def _generate_ibov_image_from_df(
    ...
    ax.plot(
        ...
        color="#153D64",
        linewidth=2.5,
        label=f"IBOV Price (last: {last_price_str})",
    )
    ax.plot(
        df_ma["Date"],
        df_ma["MA_50"],
...
def _get_ibov_technical_score(excel_obj_or_path) -> Optional[float]:
    """
    Retrieve the technical score for IBOV from 'data_technical_score' (col A, B).
    ...
    for _, row in df.iterrows():
        if str(row[df.columns[0]]).strip().upper() == "IBOV INDEX":
            try:
...
def _find_ibov_slide(prs: Presentation) -> Optional[int]:
    """Locate the index of the slide that contains the IBOV placeholder.

    This helper searches for a slide containing a shape named ``ibov`` or
    whose text is exactly ``[ibov]`` (case‑insensitive).  It returns the
    ...
    for idx, slide in enumerate(prs.slides):
        for shape in slide.shapes:
            ...
            if shape.has_text_frame:
                if (shape.text or "").strip().lower() == "[ibov]":
                    return idx
    return None


def insert_ibov_technical_score_number(prs: Presentation, excel_file) -> Presentation:
    """
    Insert the IBOV technical score (integer) into the IBOV slide.

    This function looks for a shape named ``tech_score_ibov`` on the slide
    identified by the ``ibov`` placeholder.  If not found, it searches for
    ...
    """
    score = _get_ibov_technical_score(excel_file)
    score_text = "N/A" if score is None else f"{int(round(float(score)))}"

    placeholder_name = "tech_score_ibov"
    placeholder_patterns = ["[XXX]", "XXX"]

    ibov_idx = _find_ibov_slide(prs)
    if ibov_idx is None:
        # No IBOV slide found; return unmodified
        return prs
    slide = prs.slides[ibov_idx]
    ...
    for shape in slide.shapes:
        if getattr(shape, "name", "").lower() == placeholder_name:
            ...
            return prs
    # Otherwise, search for textual placeholders within shapes on the IBOV slide
    for shape in slide.shapes:
        if shape.has_text_frame:
...
def generate_range_callout_chart_image(
    ...
    *,
    ...
    df_full : pandas.DataFrame
        Full IBOV price history with 'Date' and 'Price' columns.
    anchor_date : pandas.Timestamp or None, optional
    ...
    # value is provided (e.g. the IBOV level), use it to estimate the
    ...
    ax_chart.plot(df["Date"], df["Price"], color="#153D64", linewidth=2.5,
                  label=f"IBOV Price (last: {last_price:,.2f})")
...
def insert_ibov_technical_chart_with_callout(
    prs: Presentation,
    excel_file,
    anchor_date: Optional[pd.Timestamp] = None,
    lookback_days: int = 90,
    ...
    """
    Insert the IBOV technical analysis chart with the trading range call‑out
    into the PowerPoint.  This function mirrors the behaviour of
    ``insert_ibov_technical_chart_with_range`` but uses the call‑out style to
    ...
    excel_file : file‑like object or path
        Excel workbook containing IBOV price data.
    anchor_date : pandas.Timestamp or None, optional
    ...
    try:
        df_full = _load_price_data_from_obj(excel_file, "IBOV Index", price_mode=price_mode)
    except Exception:
        df_full = _load_price_data(pathlib.Path(excel_file), "IBOV Index", price_mode=price_mode)

    # For IBOV there is no separate volatility index (e.g. VIX).  We rely on
    # realised volatility for the trading range.  Do not compute an implied
    # volatility index value; instead pass ``None`` to the call‑out generator.
    vol_val = None
    # Generate the image with the call‑out.  Use an extended width of
    # 25.0 cm while keeping the height at 7.3 cm.  Passing ``None`` for
    # ``vol_index_value`` ensures that realised volatility is used in the
    # calculation.
...
def _get_ibov_momentum_score(excel_obj_or_path) -> Optional[float]:
    """Return IBOV momentum score, mapping letter grades to numeric if needed."""
    try:
        df = pd.read_excel(excel_obj_or_path, sheet_name="data_trend_rating")
    except Exception:
        return None
    # find IBOV row
    mask = df.iloc[:, 0].astype(str).str.strip().str.upper() == "IBOV INDEX"
    if not mask.any():
        return None
    ...
    try:
        ...
        ibov_param = params[params["Tickers"].astype(str).str.upper() == "IBOV INDEX"]
        if not ibov_param.empty and "Unnamed: 8" in ibov_param:
            return float(ibov_param["Unnamed: 8"].dropna().iloc[0])
    except Exception:
        pass
    return mapping.get(rating)


def insert_ibov_momentum_score_number(prs: Presentation, excel_file) -> Presentation:
    """
    Insert the IBOV momentum score (integer) into the IBOV slide.

    The momentum score is inserted into a shape named ``mom_score_ibov`` on
    the IBOV slide.  If that shape is not found, any ``XXX`` or ``[XXX]``
    placeholder within the IBOV slide is replaced instead.  This avoids
    inadvertently replacing placeholders on CSI or other slides.
    """
    score = _get_ibov_momentum_score(excel_file)
    score_text = "N/A" if score is None else f"{int(round(float(score)))}"

    placeholder_name = "mom_score_ibov"
    placeholder_patterns = ["[XXX]", "XXX"]
    ...
    for shape in slide.shapes:
        if getattr(shape, "name", "").lower() == placeholder_name:
            if shape.has_text_frame:
                ...
                _apply_run_font_attributes(new_run, *attrs)
            return prs
    # Otherwise, replace placeholder patterns on the IBOV slide only
    for shape in slide.shapes:
        if shape.has_text_frame:
            for pattern in placeholder_patterns:
                if pattern in (shape.text or ""):
...
def insert_ibov_technical_chart(
    prs: Presentation,
    excel_file,
    anchor_date: Optional[pd.Timestamp] = None,
    price_mode: str = "Last Price",
    ...
    """
    Insert the IBOV technical‑analysis chart into the PPT.

    We only use the textbox named ``ibov`` (or containing “[ibov]”) to locate
    ...
    try:
        ...
        df_full = _load_price_data(pathlib.Path(excel_file), "IBOV Index", price_mode=price_mode)
    img_bytes = _generate_ibov_image_from_df(df_full, anchor_date)

    # Find the slide containing the 'ibov' placeholder
    target_slide = None
    for slide in prs.slides:
        for shape in slide.shapes:
            name_attr = getattr(shape, "name", "").lower()
...
def insert_ibov_subtitle(prs: Presentation, subtitle: str) -> Presentation:
    """
    Replace the IBOV subtitle placeholder with the provided text.

    Only the slide identified by the ``ibov`` placeholder is modified.  A
    shape named ``ibov_text`` takes precedence; if it does not exist
    within the IBOV slide, any occurrences of ``XXX`` or ``[XXX]`` on
    that slide are replaced instead.  Formatting of the original run is
    preserved.
    """
    placeholder_name = "ibov_text"
    placeholder_patterns = ["[XXX]", "XXX"]
    ...
    for shape in slide.shapes:
        if getattr(shape, "name", "").lower() == placeholder_name:
            if shape.has_text_frame:
                ...
                _apply_run_font_attributes(new_run, *attrs)
            return prs
    # Otherwise, replace placeholder patterns within the IBOV slide
    for shape in slide.shapes:
        if shape.has_text_frame:
            for pattern in placeholder_patterns:
                if pattern in (shape.text or ""):
...
def insert_ibov_average_gauge(
    prs: Presentation, excel_file, last_week_avg: float
) -> Presentation:
    """
    Insert the IBOV average gauge into the IBOV slide.
    ...
    ``gauge_ibov`` within the IBOV slide.  If such a shape is not found
    within the IBOV slide, placeholders ``[GAUGE]``, ``GAUGE`` or
    ``gauge_ibov`` on the IBOV slide are used instead.  If neither is
    present, the gauge is placed at a default position below the chart
    on the IBOV slide.  Other slides remain untouched.
    """
    tech_score = _get_ibov_technical_score(excel_file)
    mom_score = _get_ibov_momentum_score(excel_file)
    ...
    if ibov_idx is None:
        return prs
    slide = prs.slides[ibov_idx]
    placeholder_name = "gauge_ibov"
    placeholder_patterns = ["[GAUGE]", "GAUGE", "gauge_ibov"]
    ...
    for shape in slide.shapes:
        if getattr(shape, "name", "").lower() == placeholder_name:
            ...
            return prs
    # Then search for textual gauge placeholders on the IBOV slide
    for shape in slide.shapes:
        if shape.has_text_frame:
            for pattern in placeholder_patterns:
                if pattern.lower() in (shape.text or "").lower():
                    ...
                    slide.shapes.add_picture(stream, left, top, width=width, height=height)
                    return prs
    # Fallback: insert below the chart within the IBOV slide using template coordinates
    left = Cm(8.97)
    top = Cm(12.13)
    width = Cm(15.15)
    height = Cm(3.13)
...
def insert_ibov_technical_assessment(
    prs: Presentation,
    excel_file,
    manual_desc: Optional[str] = None,
) -> Presentation:
    """
    Insert a descriptive assessment text into the IBOV slide.

    The assessment is written into a shape named ``ibov_view`` on the IBOV
    slide.  If no such shape exists, the function replaces any
    occurrences of ``[ibov_view]`` or ``ibov_view`` in text on that slide.
    ...
    if manual_desc is not None and isinstance(manual_desc, str):
        ...
        if desc and not desc.lower().startswith("ibov"):
            desc = f"IBOV: {desc}"
    else:
        tech_score = _get_ibov_technical_score(excel_file)
        mom_score = _get_ibov_momentum_score(excel_file)
        ...
        if avg >= 80:
            desc = "IBOV: Strongly Bullish"
        elif avg >= 70:
            desc = "IBOV: Bullish"
        elif avg >= 60:
            desc = "IBOV: Slightly Bullish"
        elif avg >= 40:
            desc = "IBOV: Neutral"
        elif avg >= 30:
            desc = "IBOV: Slightly Bearish"
        elif avg >= 20:
            desc = "IBOV: Bearish"
        else:
            desc = "IBOV: Strongly Bearish"

    target_name = "ibov_view"
    placeholder_patterns = ["[ibov_view]", "ibov_view"]

    ibov_idx = _find_ibov_slide(prs)
    if ibov_idx is None:
        return prs
    slide = prs.slides[ibov_idx]
    # Try to locate a shape by name on the IBOV slide
    for shape in slide.shapes:
        ...
        if name_attr and name_attr.lower() == target_name:
            if shape.has_text_frame:
                ...
                _apply_run_font_attributes(new_run, *attrs)
            return prs
    # Otherwise, replace placeholder patterns on the IBOV slide
    for shape in slide.shapes:
        if shape.has_text_frame:
            for pattern in placeholder_patterns:
                if pattern.lower() in (shape.text or "").lower():
...
def insert_ibov_source(
    prs: Presentation,
    used_date: Optional[pd.Timestamp],
    price_mode: str,
) -> Presentation:
    """
    Insert the source footnote into a shape named 'ibov_source' (or
    containing '[ibov_source]').  The footnote text depends on the selected
    ...
    placeholder_name = "ibov_source"
    placeholder_patterns = ["[ibov_source]", "ibov_source"]
    # Restrict insertion to the IBOV slide only
    ibov_idx = _find_ibov_slide(prs)
    if ibov_idx is None:
        return prs
    slide = prs.slides[ibov_idx]
    ...
    for shape in slide.shapes:
        ...
        if name_attr and name_attr.lower() == placeholder_name:
            if shape.has_text_frame:
                ...
                _apply_run_font_attributes(new_run, *attrs)
            return prs
    # Case 2: replace occurrences of the placeholder pattern in text on the IBOV slide
    for shape in slide.shapes:
        if shape.has_text_frame:
            for pattern in placeholder_patterns:
                if pattern.lower() in (shape.text or "").lower():
...
def _compute_range_bounds(
    ...
    """
    Compute fallback high and low range bounds for the IBOV using
    realised volatility.

    This helper is used when an implied volatility index (e.g. IBOV) is
...
def generate_range_gauge_chart_image(
    ...
    *,
    ...
    """
    Create a PNG image of the IBOV price chart with a vertical range gauge
    ...
    ----------
    df_full : pandas.DataFrame
        Full IBOV price history as returned by ``_load_price_data``.
    anchor_date : pandas.Timestamp or None, optional
    ...
    ax.plot(
        df["Date"], df["Price"], color="#153D64", linewidth=2.5, label=f"IBOV Price (last: {last_price_str})"
    )
...
def insert_ibov_technical_chart_with_range(
    prs: Presentation,
    excel_file,
    anchor_date: Optional[pd.Timestamp] = None,
    lookback_days: int = 90,
    ...
    """
    Insert the IBOV technical analysis chart with the vertical range gauge into the PPT.

    This function behaves similarly to ``insert_ibov_technical_chart`` but uses
    ``generate_range_gauge_chart_image`` to draw a combined chart and gauge.
    It attempts to find a shape named 'ibov' or containing '[ibov]' to locate the
    ...
    try:
        ...
        df_full = _load_price_data(pathlib.Path(excel_file), "IBOV Index", price_mode=price_mode)
    # For IBOV there is no separate volatility index (e.g. VIX).  We rely
    # solely on realised volatility to determine the expected trading range.
    # Do not pass a volatility index value to ``generate_range_gauge_chart_image``; this
    # forces the function to compute the range from historical price moves.
    vol_val = None
    ...
    # The IBOV slide in the template allocates ~21.41 cm for the chart area
