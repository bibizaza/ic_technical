"""
Refactored Mexbol technical analysis module using base instrument class.
...
- Reduced code from 2248+ lines to ~300 lines
"""
...
# Import MARS scorer
try:
    from mars_engine.mars_lite_scorer import generate_mexbol_score_history
except Exception:
    generate_mexbol_score_history = None
...
_config = InstrumentConfig(
    ...
    mars_scorer_func=generate_mexbol_score_history,
)
...
def insert_mexbol_technical_chart(
    ...
    """Insert a technical chart into the Mexbol slide."""
...
def insert_mexbol_momentum_score_number(
    prs: Presentation,
    excel_file,
    price_mode: str = "Last Price",
) -> Presentation:
...
def insert_mexbol_subtitle(prs: Presentation, subtitle: str) -> Presentation:
    """Insert a subtitle into the Mexbol slide."""
...
def _get_mexbol_momentum_score(
    excel_obj_or_path,
    price_mode: str = "Last Price",
) -> Optional[float]:
    """Retrieve the MARS momentum score for Mexbol."""
    return _instrument._get_momentum_score(excel_obj_or_path, price_mode)


# Stub functions for compatibility
def insert_mexbol_technical_chart_with_callout(prs: Presentation, *args, **kwargs) -> Presentation:
    """Insert chart with callout (stub)."""
    return insert_mexbol_technical_chart(prs, *args, **kwargs)


def insert_mexbol_technical_chart_with_range(prs: Presentation, *args, **kwargs) -> Presentation:
    """Insert chart with range gauge (stub)."""
    return insert_mexbol_technical_chart(prs, *args, **kwargs)


def insert_mexbol_average_gauge(prs: Presentation, *args, **kwargs) -> Presentation:
    """Insert average gauge (stub)."""
    return prs


def insert_mexbol_technical_assessment(prs: Presentation, *args, **kwargs) -> Presentation:
    """Insert technical assessment (stub)."""
    return prs


def insert_mexbol_source(prs: Presentation, *args, **kwargs) -> Presentation:
