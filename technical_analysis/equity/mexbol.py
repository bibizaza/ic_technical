...
def _get_mexbol_technical_score(excel_obj_or_path) -> Optional[float]:
    """
    Retrieve the technical score for MEXBOL from 'data_technical_score' (col A, B).
    ...
    for _, row in df.iterrows():
        if str(row[df.columns[0]]).strip().upper() == "MEXBOL INDEX":
            try:
...
def _find_mexbol_slide(prs: Presentation) -> Optional[int]:
    """Locate the index of the slide that contains the MEXBOL placeholder.

    This helper searches for a slide containing a shape named ``mexbol`` or
    whose text is exactly ``[mexbol]`` (case‑insensitive).  It returns the
...
def insert_mexbol_technical_score_number(prs: Presentation, excel_file) -> Presentation:
    """
    Insert the MEXBOL technical score (integer) into the MEXBOL slide.

    This function looks for a shape named ``tech_score_mexbol`` on the slide
    identified by the ``mexbol`` placeholder.  If not found, it searches for
    ...
    for shape in slide.shapes:
        if getattr(shape, "name", "").lower() == placeholder_name:
            ...
            return prs
    # Otherwise, search for textual placeholders within shapes on the MEXBOL slide
    for shape in slide.shapes:
        if shape.has_text_frame:
...
def _get_mexbol_momentum_score(excel_obj_or_path) -> Optional[float]:
    """Return MEXBOL momentum score, mapping letter grades to numeric if needed."""
    try:
        df = pd.read_excel(excel_obj_or_path, sheet_name="data_trend_rating")
    except Exception:
        return None
    # find MEXBOL row
    mask = df.iloc[:, 0].astype(str).str.strip().str.upper() == "MEXBOL INDEX"
    if not mask.any():
        return None
    ...
    try:
        ...
        mexbol_param = params[params["Tickers"].astype(str).str.upper() == "MEXBOL INDEX"]
        if not mexbol_param.empty and "Unnamed: 8" in mexbol_param:
            return float(mexbol_param["Unnamed: 8"].dropna().iloc[0])
    except Exception:
        pass
...
def insert_mexbol_momentum_score_number(prs: Presentation, excel_file) -> Presentation:
    """
    Insert the MEXBOL momentum score (integer) into the MEXBOL slide.

    The momentum score is inserted into a shape named ``mom_score_mexbol`` on
    the MEXBOL slide.  If that shape is not found, any ``XXX`` or ``[XXX]``
    placeholder within the MEXBOL slide is replaced instead.  This avoids
    inadvertently replacing placeholders on CSI or other slides.
    """
    score = _get_mexbol_momentum_score(excel_file)
    score_text = "N/A" if score is None else f"{int(round(float(score)))}"

    placeholder_name = "mom_score_mexbol"
    placeholder_patterns = ["[XXX]", "XXX"]
    ...
    for shape in slide.shapes:
        if getattr(shape, "name", "").lower() == placeholder_name:
            if shape.has_text_frame:
                ...
                _apply_run_font_attributes(new_run, *attrs)
            return prs
    # Otherwise, replace placeholder patterns on the MEXBOL slide only
    for shape in slide.shapes:
        if shape.has_text_frame:
            for pattern in placeholder_patterns:
                if pattern in (shape.text or ""):
...
def insert_mexbol_subtitle(prs: Presentation, subtitle: str) -> Presentation:
    """
    Replace the MEXBOL subtitle placeholder with the provided text.

    Only the slide identified by the ``mexbol`` placeholder is modified.  A
    shape named ``mexbol_text`` takes precedence; if it does not exist
    within the MEXBOL slide, any occurrences of ``XXX`` or ``[XXX]`` on
    that slide are replaced instead.  Formatting of the original run is
    preserved.
    """
    placeholder_name = "mexbol_text"
    placeholder_patterns = ["[XXX]", "XXX"]
    ...
    for shape in slide.shapes:
        if getattr(shape, "name", "").lower() == placeholder_name:
            if shape.has_text_frame:
                ...
                _apply_run_font_attributes(new_run, *attrs)
            return prs
    # Otherwise, replace placeholder patterns within the MEXBOL slide
    for shape in slide.shapes:
        if shape.has_text_frame:
            for pattern in placeholder_patterns:
                if pattern in (shape.text or ""):
...
def _compute_range_bounds(
    ...
    """
    Compute fallback high and low range bounds for the Mexbol using
    realised volatility.
