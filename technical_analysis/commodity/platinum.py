"""
Utility functions for Platinum technical analysis and high‑resolution export.

This module provides tools to build interactive and static charts for the
Platinum index, calculate and insert technical and momentum scores into
...
* ``make_platinum_figure`` – interactive Plotly chart for Streamlit.
* ``insert_platinum_technical_chart`` – insert a static Platinum chart into a PPTX.
* ``insert_platinum_technical_score_number`` – insert the technical score (integer).
* ``insert_platinum_momentum_score_number`` – insert the momentum score (integer).
* ``insert_platinum_subtitle`` – insert a user‑defined subtitle into the Platinum slide.
* ``generate_average_gauge_image`` – create a horizontal gauge image.
* ``insert_platinum_average_gauge`` – insert the gauge into a PPT slide.
* ``insert_platinum_technical_assessment`` – insert a descriptive “view” text.
* ``generate_range_gauge_chart_image`` – create a combined price chart with
  ...
  ``insert_platinum_technical_chart_with_range``.
* ``insert_platinum_technical_chart_with_range`` – insert the Platinum technical
  analysis chart with the higher/lower range gauge into the PPT.

The range gauge illustrates the recent trading range for the Platinum.
...
code looks up the forward‑looking volatility index (XPTUSDV1M BGN Curncy) and computes a
1‑week expected move as ``(current_price × (XPTUSDV1M BGN Curncy / 100)) / sqrt(52)``.
...
def _get_vol_index_value(
    excel_obj_or_path,
    price_mode: str = "Last Price",
    vol_ticker: str = "XPTUSDV1M BGN Curncy",
) -> Optional[float]:
    """
    Retrieve the most recent value of a volatility index (e.g. XPTUSDV1M BGN Curncy) from
    ...
    vol_ticker : str, default "XPTUSDV1M BGN Curncy"
...
def make_platinum_figure(
    excel_path: str | pathlib.Path,
    anchor_date: Optional[pd.Timestamp] = None,
    price_mode: str = "Last Price",
) -> go.Figure:
    """
    Build an interactive Platinum chart for Streamlit.

    Parameters
    ----------
    excel_path : str or pathlib.Path
        Path to the Excel file containing Platinum price data.
    ...
    df_raw = _load_price_data(excel_path, "XPT Comdty", price_mode=price_mode)
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=df["Price"],
            mode="lines",
            name=f"Platinum Price (last: {last_price_str})",
...
def _generate_platinum_image_from_df(
    ...
    ax.plot(
        ...
        color="#153D64",
        linewidth=2.5,
        label=f"Platinum Price (last: {last_price_str})",
    )
    ax.plot(
        df_ma["Date"],
        df_ma["MA_50"],
...
def _find_platinum_slide(prs: Presentation) -> Optional[int]:
    """Locate the index of the slide that contains the Platinum placeholder.

    This helper searches for a slide containing a shape named ``platinum`` or
    whose text is exactly ``[platinum]`` (case‑insensitive).  It returns the
    zero‑based slide index or ``None`` if no such slide exists.
    """
    for idx, slide in enumerate(prs.slides):
        for shape in slide.shapes:
            name_attr = getattr(shape, "name", "").lower()
            if name_attr == "platinum":
                return idx
            if shape.has_text_frame:
                if (shape.text or "").strip().lower() == "[platinum]":
                    return idx
    return None


def insert_platinum_technical_score_number(prs: Presentation, excel_file) -> Presentation:
    """
    Insert the Platinum technical score (integer) into the Platinum slide.

    This function looks for a shape named ``tech_score_platinum`` on the slide
    identified by the ``platinum`` placeholder.  If not found, it searches for
    ...
    """
    score = _get_platinum_technical_score(excel_file)
    score_text = "N/A" if score is None else f"{int(round(float(score)))}"

    placeholder_name = "tech_score_platinum"
    placeholder_patterns = ["[XXX]", "XXX"]

    platinum_idx = _find_platinum_slide(prs)
    if platinum_idx is None:
        # No Platinum slide found; return unmodified
        return prs
    slide = prs.slides[platinum_idx]
    ...
    for shape in slide.shapes:
        if getattr(shape, "name", "").lower() == placeholder_name:
            ...
            return prs
    # Otherwise, search for textual placeholders within shapes on the Platinum slide
    for shape in slide.shapes:
        if shape.has_text_frame:
...
def generate_range_callout_chart_image(
    ...
    *,
    ...
    """
    Create a PNG image of the Platinum price chart with a textual call‑out on the
    ...
    df_full : pandas.DataFrame
        Full Platinum price history with 'Date' and 'Price' columns.
    anchor_date : pandas.Timestamp or None, optional
    ...
    # value is provided (e.g. the XPTUSDV1M BGN Curncy level), use it to estimate the
    ...
    ax_chart.plot(df["Date"], df["Price"], color="#153D64", linewidth=2.5,
                  label=f"Platinum Price (last: {last_price:,.2f})")
...
def insert_platinum_technical_chart_with_callout(
    prs: Presentation,
    excel_file,
    anchor_date: Optional[pd.Timestamp] = None,
    lookback_days: int = 90,
    ...
    """
    Insert the Platinum technical analysis chart with the trading range call‑out
    into the PowerPoint.  This function mirrors the behaviour of
    ``insert_platinum_technical_chart_with_range`` but uses the call‑out style to
    ...
    excel_file : file‑like object or path
        Excel workbook containing Platinum price data.
    anchor_date : pandas.Timestamp or None, optional
    ...
    try:
        df_full = _load_price_data_from_obj(excel_file, "XPT Comdty", price_mode=price_mode)
    except Exception:
        df_full = _load_price_data(pathlib.Path(excel_file), "XPT Comdty", price_mode=price_mode)

    # Determine the implied volatility index value (XPTUSDV1M BGN Curncy) from the Excel file
    ...
    vol_val = _get_vol_index_value(excel_file, price_mode=price_mode, vol_ticker="XPTUSDV1M BGN Curncy")
    ...
    # Locate the slide containing the 'platinum' placeholder or text
    target_slide = None
    ...
    try:
        ...
        pass
    # ------------------------------------------------------------------
    # Replace the last price placeholder on the Platinum slide.  Compute
    # the most recent price from the full DataFrame; if the data is
    # missing, fall back to 'N/A'.  The placeholder may appear as a
    # shape named 'last_price_platinum' or within the text (e.g.
    # '[last_price_platinum]' in a manually added legend).  Preserve the
    ...
    placeholder_name = "last_price_platinum"
    placeholder_patterns = ["[last_price_platinum]", "last_price_platinum"]
    replaced = False
...
def insert_platinum_momentum_score_number(prs: Presentation, excel_file) -> Presentation:
    """
    Insert the Platinum momentum score (integer) into the Platinum slide.

    The momentum score is inserted into a shape named ``mom_score_platinum`` on
    the Platinum slide.  If that shape is not found, any ``XXX`` or ``[XXX]``
    placeholder within the Platinum slide is replaced instead.  This avoids
    inadvertently replacing placeholders on CSI or other slides.
    """
    score = _get_platinum_momentum_score(excel_file)
    score_text = "N/A" if score is None else f"{int(round(float(score)))}"

    placeholder_name = "mom_score_platinum"
    placeholder_patterns = ["[XXX]", "XXX"]
    ...
    for shape in slide.shapes:
        if getattr(shape, "name", "").lower() == placeholder_name:
            if shape.has_text_frame:
                ...
                _apply_run_font_attributes(new_run, *attrs)
            return prs
    # Otherwise, replace placeholder patterns on the Platinum slide only
    for shape in slide.shapes:
        if shape.has_text_frame:
            for pattern in placeholder_patterns:
                if pattern in (shape.text or ""):
...
def insert_platinum_technical_chart(
    prs: Presentation,
    excel_file,
    anchor_date: Optional[pd.Timestamp] = None,
    price_mode: str = "Last Price",
    ...
    """
    Insert the Platinum technical‑analysis chart into the PPT.

    We only use the textbox named ``platinum`` (or containing “[platinum]”) to locate
    ...
    try:
        ...
        df_full = _load_price_data(pathlib.Path(excel_file), "XPT Comdty", price_mode=price_mode)
    img_bytes = _generate_platinum_image_from_df(df_full, anchor_date)

    # Find the slide containing the 'platinum' placeholder
    target_slide = None
    for slide in prs.slides:
        for shape in slide.shapes:
            name_attr = getattr(shape, "name", "").lower()
...
def insert_platinum_subtitle(prs: Presentation, subtitle: str) -> Presentation:
    """
    Replace the Platinum subtitle placeholder with the provided text.

    Only the slide identified by the ``platinum`` placeholder is modified.  A
    shape named ``platinum_text`` takes precedence; if it does not exist
    within the Platinum slide, any occurrences of ``XXX`` or ``[XXX]`` on
    that slide are replaced instead.  Formatting of the original run is
    preserved.
    """
    placeholder_name = "platinum_text"
    placeholder_patterns = ["[XXX]", "XXX"]
    ...
    for shape in slide.shapes:
        if getattr(shape, "name", "").lower() == placeholder_name:
            if shape.has_text_frame:
                ...
                _apply_run_font_attributes(new_run, *attrs)
            return prs
    # Otherwise, replace placeholder patterns within the Platinum slide
    for shape in slide.shapes:
        if shape.has_text_frame:
            for pattern in placeholder_patterns:
                if pattern in (shape.text or ""):
...
def insert_platinum_average_gauge(
    prs: Presentation, excel_file, last_week_avg: float
) -> Presentation:
    """
    Insert the Platinum average gauge into the Platinum slide.
    ...
    ``gauge_platinum`` within the Platinum slide.  If such a shape is not found
    within the Platinum slide, placeholders ``[GAUGE]``, ``GAUGE`` or
    ``gauge_platinum`` on the Platinum slide are used instead.  If neither is
    present, the gauge is placed at a default position below the chart
    on the Platinum slide.  Other slides remain untouched.
    """
    tech_score = _get_platinum_technical_score(excel_file)
    mom_score = _get_platinum_momentum_score(excel_file)
    ...
    try:
        ...
        return prs
    # Identify Platinum slide
    platinum_idx = _find_platinum_slide(prs)
    if platinum_idx is None:
        return prs
    slide = prs.slides[platinum_idx]
    placeholder_name = "gauge_platinum"
    placeholder_patterns = ["[GAUGE]", "GAUGE", "gauge_platinum"]
    ...
    for shape in slide.shapes:
        if getattr(shape, "name", "").lower() == placeholder_name:
            ...
            return prs
    # Then search for textual gauge placeholders on the Platinum slide
    for shape in slide.shapes:
        if shape.has_text_frame:
            for pattern in placeholder_patterns:
                if pattern.lower() in (shape.text or "").lower():
                    ...
                    slide.shapes.add_picture(stream, left, top, width=width, height=height)
                    return prs
    # Fallback: insert below the chart within the Platinum slide using template coordinates
    left = Cm(8.97)
    top = Cm(12.13)
    width = Cm(15.15)
    height = Cm(3.13)
...
def insert_platinum_technical_assessment(
    prs: Presentation,
    excel_file,
    manual_desc: Optional[str] = None,
) -> Presentation:
    """
    Insert a descriptive assessment text into the Platinum slide.

    The assessment is written into a shape named ``platinum_view`` on the Platinum
    slide.  If no such shape exists, the function replaces any
    occurrences of ``[platinum_view]`` or ``platinum_view`` in text on that slide.
    ...
    if manual_desc is not None and isinstance(manual_desc, str):
        ...
        if avg >= 80:
            desc = "Platinum: Strongly Bullish"
        elif avg >= 70:
            desc = "Platinum: Bullish"
        elif avg >= 60:
            desc = "Platinum: Slightly Bullish"
        elif avg >= 40:
            desc = "Platinum: Neutral"
        elif avg >= 30:
            desc = "Platinum: Slightly Bearish"
        elif avg >= 20:
            desc = "Platinum: Bearish"
        else:
            desc = "Platinum: Strongly Bearish"

    target_name = "platinum_view"
    placeholder_patterns = ["[platinum_view]", "platinum_view"]

    platinum_idx = _find_platinum_slide(prs)
    if platinum_idx is None:
        return prs
    slide = prs.slides[platinum_idx]
    # Try to locate a shape by name on the Platinum slide
    for shape in slide.shapes:
        ...
        if name_attr and name_attr.lower() == target_name:
            if shape.has_text_frame:
                ...
                _apply_run_font_attributes(new_run, *attrs)
            return prs
    # Otherwise, replace placeholder patterns on the Platinum slide
    for shape in slide.shapes:
        if shape.has_text_frame:
            for pattern in placeholder_patterns:
                if pattern.lower() in (shape.text or "").lower():
...
def insert_platinum_source(
    prs: Presentation,
    used_date: Optional[pd.Timestamp],
    price_mode: str,
) -> Presentation:
    """
    Insert the source footnote into a shape named 'platinum_source' (or
    containing '[platinum_source]').  The footnote text depends on the selected
    ...
    placeholder_name = "platinum_source"
    placeholder_patterns = ["[platinum_source]", "platinum_source"]
    # Restrict insertion to the Platinum slide only
    platinum_idx = _find_platinum_slide(prs)
    if platinum_idx is None:
        return prs
    slide = prs.slides[platinum_idx]
    ...
    for shape in slide.shapes:
        ...
        if name_attr and name_attr.lower() == placeholder_name:
            if shape.has_text_frame:
                ...
                _apply_run_font_attributes(new_run, *attrs)
            return prs
    # Case 2: replace occurrences of the placeholder pattern in text on the Platinum slide
    for shape in slide.shapes:
        if shape.has_text_frame:
            for pattern in placeholder_patterns:
                if pattern.lower() in (shape.text or "").lower():
...
def generate_range_gauge_chart_image(
    ...
    *,
    ...
    """
    Create a PNG image of the Platinum price chart with a vertical range gauge
    ...
    ----------
    df_full : pandas.DataFrame
        Full Platinum price history as returned by ``_load_price_data``.
    anchor_date : pandas.Timestamp or None, optional
    ...
    ax.plot(
        df["Date"], df["Price"], color="#153D64", linewidth=2.5, label=f"Platinum Price (last: {last_price_str})"
    )
...
def insert_platinum_technical_chart_with_range(
    prs: Presentation,
    excel_file,
    anchor_date: Optional[pd.Timestamp] = None,
    lookback_days: int = 90,
    ...
    """
    Insert the Platinum technical analysis chart with the vertical range gauge into the PPT.

    This function behaves similarly to ``insert_platinum_technical_chart`` but uses
    ``generate_range_gauge_chart_image`` to draw a combined chart and gauge.
    It attempts to find a shape named 'platinum' or containing '[platinum]' to locate the
    ...
    # The Platinum slide in the template allocates ~21.41 cm for the chart area
