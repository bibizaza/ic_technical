...
def _get_copper_momentum_score(excel_obj_or_path) -> Optional[float]:
    ...
    try:
        ...
        copper_param = params[params["Tickers"].astype(str).str.upper() == "LP1 COMDTY"]
        if not copper_param.empty and "Unnamed: 8" in copper_param:
            return float(copper_param["Unnamed: 8"].dropna().iloc[0])
    except Exception:
        pass
