...
def _get_gold_technical_score(excel_obj_or_path) -> Optional[float]:
    """
    Retrieve the technical score for Gold from 'data_technical_score' (col A, B).
    ...
    for _, row in df.iterrows():
        # Search for the Gold ticker (GCA COMDTY) rather than the original SPX string
        if str(row[df.columns[0]]).strip().upper() == "GCA COMDTY":
            try:
