...
def _get_ethereum_momentum_score(excel_obj_or_path) -> Optional[float]:
    ...
    try:
        ...
        eth_param = params[params["Tickers"].astype(str).str.upper() == "XETUSD CURNCY"]
        if not eth_param.empty and "Unnamed: 8" in eth_param.columns:
            custom_series = eth_param["Unnamed: 8"].dropna()
