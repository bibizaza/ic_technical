...
def _get_ripple_technical_score(excel_obj_or_path) -> Optional[float]:
    """
    Retrieve the technical score for RIPPLE.
    Uses common helper with instrument-specific ticker.
    """
    return _get_technical_score_generic(excel_obj_or_path, "XRPUSD CURNCY")



def _find_ripple_slide(prs: Presentation) -> Optional[int]:
    """Find the Ripple slide by placeholder."""
    return find_slide_by_placeholder(prs, "ripple")



def insert_ripple_technical_score_number(prs: Presentation, excel_file) -> Presentation:
    """Insert the Ripple technical score into the slide."""
    score = _get_ripple_technical_score(excel_file)
    return insert_score_number(prs, score, "ripple", "tech_score")
...
def generate_range_callout_chart_image(
    ...
    *,
    ...
    def _fmt(val: float) -> str:
        """Format a numeric value with thousands separators and two decimals.

        Ripple prices are often less than 1, so include two decimal
        places for clarity.  Commas are replaced with apostrophes to
        match the template style.
        """
        try:
            return f"{val:,.2f}".replace(",", "'")
        except Exception:
            # Fallback: format with two decimals
            return f"{val:.2f}"
...
def insert_ripple_technical_chart_with_callout(
    ...
    try:
        ...
        pass
    # ------------------------------------------------------------------
    # Replace the last price placeholder on the Ripple slide.  Compute
    # the most recent price from the full DataFrame; if the data is
    # missing, fall back to 'N/A'.  The placeholder may appear as a
    # shape named 'last_price_ripple' or within the text (e.g.
    # '[last_price_ripple]' in the manually added legend).  Preserve the
    ...
    placeholder_name = "last_price_ripple"
    placeholder_patterns = ["[last_price_ripple]", "last_price_ripple"]
    replaced = False
...
def _get_ripple_momentum_score(excel_obj_or_path) -> Optional[float]:
    """
    Retrieve the momentum score for RIPPLE.
    Uses common helper with instrument-specific ticker.
    """
    return _get_momentum_score_generic(excel_obj_or_path, "XRPUSD CURNCY")



def insert_ripple_momentum_score_number(prs: Presentation, excel_file) -> Presentation:
    """Insert the Ripple momentum score into the slide."""
    score = _get_ripple_momentum_score(excel_file)
    return insert_score_number(prs, score, "ripple", "momentum_score")
...
def insert_ripple_subtitle(prs: Presentation, subtitle: str) -> Presentation:
    """Insert subtitle into the Ripple slide."""
    return insert_subtitle(prs, subtitle, "ripple")
