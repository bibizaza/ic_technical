"""
Refactored BNB technical analysis module using base instrument class.
...
- Reduced code from 2228+ lines to ~300 lines
"""
...
# Import MARS scorer
try:
    from mars_engine.mars_lite_scorer import generate_binance_score_history
except Exception:
    generate_binance_score_history = None

# Configuration
PLOT_LOOKBACK_DAYS: int = 90

# Peer group for relative momentum
PEER_GROUP = ['XBTUSD Curncy', 'XETUSD Curncy', 'XBNCUR Curncy', 'SOLUSD Curncy', 'XRPUSD Curncy', 'SPX Index', 'GCA Comdty']
...
_config = InstrumentConfig(
    ...
    vol_ticker='DVOL Index',
    peer_group=PEER_GROUP,
    mars_scorer_func=generate_binance_score_history,
)
...
def make_binance_figure(
    ...
    """Build an interactive BNB chart for Streamlit."""
...
def insert_binance_technical_chart(
    ...
    """Insert a technical chart into the BNB slide."""
...
def insert_binance_technical_score_number(prs: Presentation, excel_file) -> Presentation:
    """Insert the BNB technical score into the slide."""
    return _instrument.insert_technical_score_number(prs, excel_file)


def insert_binance_momentum_score_number(
    prs: Presentation,
    excel_file,
    price_mode: str = "Last Price",
) -> Presentation:
    """Insert the BNB momentum score into the slide."""
...
def insert_binance_subtitle(prs: Presentation, subtitle: str) -> Presentation:
    """Insert a subtitle into the BNB slide."""
    return _instrument.insert_subtitle(prs, subtitle)


def _get_binance_technical_score(excel_obj_or_path) -> Optional[float]:
    """Retrieve the technical score for BNB."""
    return _instrument._get_technical_score(excel_obj_or_path)


def _get_binance_momentum_score(
    excel_obj_or_path,
    price_mode: str = "Last Price",
) -> Optional[float]:
    """Retrieve the MARS momentum score for BNB."""
    return _instrument._get_momentum_score(excel_obj_or_path, price_mode)


# Stub functions for compatibility
def insert_binance_technical_chart_with_callout(prs: Presentation, *args, **kwargs) -> Presentation:
    """Insert chart with callout (stub)."""
    return insert_binance_technical_chart(prs, *args, **kwargs)


def insert_binance_technical_chart_with_range(prs: Presentation, *args, **kwargs) -> Presentation:
    """Insert chart with range gauge (stub)."""
    return insert_binance_technical_chart(prs, *args, **kwargs)


def insert_binance_average_gauge(prs: Presentation, *args, **kwargs) -> Presentation:
    """Insert average gauge (stub)."""
    return prs


def insert_binance_technical_assessment(prs: Presentation, *args, **kwargs) -> Presentation:
    """Insert technical assessment (stub)."""
    return prs


def insert_binance_source(prs: Presentation, *args, **kwargs) -> Presentation:
