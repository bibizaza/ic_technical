#!/usr/bin/env python3
"""
Phase 4 Refactoring: Move identical chart/image functions to common module

This script moves truly identical functions (like generate_average_gauge_image)
from all instruments to common_helpers.py, eliminating massive duplication.

Expected savings: ~2,774+ lines for generate_average_gauge_image alone
"""
...
for category in ['equity', 'commodity', 'crypto']:
    ...
    for file in category_path.glob('*.py'):
        if not file.name.endswith('_backup.py'):
            instruments.append(file)

# Functions that are IDENTICAL across all instruments (can be moved to common)
IDENTICAL_FUNCTIONS = [
...
def refactor_instrument_phase4(file_path: Path):
    """Remove identical functions and add import."""
    ...
    # Create backup
    backup_path = file_path.with_name(file_path.stem + '_phase4_backup.py')
    shutil.copy(file_path, backup_path)

    # Update imports to include the identical functions
    ...
    for func_name in IDENTICAL_FUNCTIONS:
        # Match function from def to next def
...
def main():
    ...
    print("=" * 80)
    print("PHASE 4 REFACTORING - Move Identical Chart Functions")
    print("=" * 80)
    print()
    print("Moving identical functions to common_helpers.py:")
    ...
    for file_path in sorted(instruments):
        reduction = refactor_instrument_phase4(file_path)
    ...
    print("=" * 80)
    print(f"Phase 4 lines removed:  {total_reduction:,}")
    print()
    print(f"CUMULATIVE TOTALS:")
    print(f"  Phase 1: 2,322 lines")
    print(f"  Phase 2: 2,400 lines")
    print(f"  Phase 3: 2,795 lines")
    print(f"  Phase 4: {total_reduction:,} lines")
    print(f"  Total:   {7517 + total_reduction:,} lines removed")
    print()
